...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	// Query the database to check if the username and password exist in the users table
	...
	if (mysqli_num_rows($result) == 1) {
		...
		elseif($account_type == 'lab_technician') {
		...
		elseif($account_type == 'cashier') {
			...
			// Query the database to check if the user is assigned to any patients
			$sql = "SELECT * FROM assigned WHERE d_id='$user_id'";
			$result = mysqli_query($conn, $sql);

			if (mysqli_num_rows($result) > 0) {
				echo '<div class="row vh-100 bg-light rounded align-items-center justify-content-center mx-0">';
