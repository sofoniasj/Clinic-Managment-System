...
// Fetch lab test details from lab_test_view where id matches test_id from POST
$sql = "SELECT id, p_id, instruction, result FROM lab_test_view WHERE id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $_POST['test_id']);
$stmt->execute();
$stmt->bind_result($test_id, $p_id, $instruction, $result);
$stmt->fetch();
?>
...
<!-- Form to insert diagnosis -->
<form action="insert_diagno.php" method="post">
  <input type="hidden" name="test_id" value="<?php echo $test_id; ?>">
  <label for="diagnosis">Diagnosis:</label>

  

  <input type="text" class="form-control" name="diagnosis" id="diagnosis" style="height: 150px;" required>
