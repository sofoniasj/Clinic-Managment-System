...
$sql = "INSERT INTO diagnosis (p_id, diagnosis) VALUES (?, ?)";
$stmt = $conn->prepare($sql);
$stmt->bind_param("is", $_POST['test_id'], $_POST['diagnosis']);
$stmt->execute();

// Move lab test record from lab_test_view to lab_test_store
$sql = "INSERT INTO lab_test_store (p_id, instruction, result) SELECT p_id, instruction, result FROM lab_test_view WHERE id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $_POST['test_id']);
$stmt->execute();
$sql = "DELETE FROM lab_test_view WHERE id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $_POST['test_id']);
$stmt->execute();

echo "<script>window.location.href = 'diagnosis.php';</script>";
